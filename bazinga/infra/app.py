...
from bazinga.infra.route import Route
from bazinga.infra.handler import handle_request
from bazinga.infra.route import dictify_routes
from bazinga.infra.exceptions import RouteNotFound
...
class App:
    ...
    async def send_404(self, exception: RouteNotFound, send):
        ...
        await send({
            ...
            'body': orjson.dumps(
                {
                    'message': f'Rota {exception.verb} {exception.path} não encontrada.',
                    'verb': exception.verb,
                    'path': exception.path,
                })
        })
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            route = self.get_route_from_path(
                verb=scope['method'], path=scope['path'])
