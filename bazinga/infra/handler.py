...
from bazinga.infra.databases import Databases
...
class Request:
    ...
    def query_string(self):
        ...
        return dict(
            (k.decode(), [item.decode()
                          for item in v] if len(v) > 1 else v[0].decode())
...
async def handle_request(route: Route, scope, receive, send):
    ...
    resp = await next(handlers_iterator)(request, response, handlers_iterator)